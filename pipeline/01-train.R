...
# ensure the recipe is trained and fully self-contained
dir.create(dirname(paths$output$workflow_recipe$local), recursive = TRUE, showWarnings = FALSE)

trained_recipe <- lgbm_wflow_final_full_fit %>%
  workflows::extract_recipe() %>%
  recipes::prep(training = training_data_full, retain = TRUE, verbose = FALSE)
...
# Save the entire fitted workflow (contains preproc + parsnip fit)
saveRDS(lgbm_wflow_final_full_fit, file = "output/workflow/fitted_workflow.rds")

# Try to extract the native LightGBM booster and save it too
pf  <- workflows::extract_fit_parsnip(lgbm_wflow_final_full_fit)
eng <- try(parsnip::extract_fit_engine(pf), silent = TRUE)

if (!inherits(eng, "try-error") && inherits(eng, "lgb.Booster")) {
  saveRDS(eng, file = "output/model/lightgbm_booster.rds")
  message("Saved LightGBM booster to output/model/lightgbm_booster.rds")
} else if (!is.null(pf$fit) && inherits(pf$fit, "lgb.Booster")) {
  saveRDS(pf$fit, file = "output/model/lightgbm_booster.rds")
  message("Saved LightGBM booster to output/model/lightgbm_booster.rds")
} else {
